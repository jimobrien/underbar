...
(function () {
  ...
  _.defaults = function (obj) {
    // Lots of repeated code here.. look into cleaning it up a bit later
    _.each(arguments, function (item) {
      if (typeof item === 'object') {
        _.each(item, function (val, prop) {
  ...
  _.shuffle = function (array) {
    var copy = [].slice.call(array, 0); // clone the array

    // copy the array
    // begin a loop through the array
      // generate a random index
      // store the value found at that index in a temp var
      // store the value found at [i] in a var
      // swap the two
      // repeat 
    _.each(copy, function (item, i) {
      var temp;
      var temp2;
      var rand = Math.floor(Math.round() * copy.length);
  ...
  _.sortBy = function (collection, iterator) {
    ...
    // check iterator for string type
    // 
  };
  ...
  _.zip = function () {
    ...
    }); // move the longest array to a known index

    _.each(args[0], function (item, i) {
      var temp = [];
    ...
    // loop through the args
      //  push the [i] of each array into a temp array
      //  [1,2,3], [1,2,3]
    //  
  };
